...
function InputSelect(props) {
  ...
  const select = (
    <div className="input-top">
      ...
      <Button id={`${label}-input`} className={`input-select input-select--active ${isActive === '' && 'input-select--placeholder'}`} onClick={() => toggleOptions()}>
        <span className="input-select-content">
          ...
          <svg className={`${isOptionsActive ? 'select arrow select-arrow-up' : 'select-arrow'}`} width="15" height="9" viewBox="0 0 15 9" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path className="select-toggle" d="M14.25 1.25L7.75 7.75L1.25 1.25" strokeWidth="1.5" strokeLinejoin="round" />
          </svg>
        </span>
      </Button>
      {isOptionsActive && (
